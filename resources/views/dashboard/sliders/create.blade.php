...
@push('before-css')
    ...
    <link rel="stylesheet" href="{{asset('assets/dashboard/styles/vendor/dropzone.min.css')}}">
                    ...
                    <ul class="nav nav-tabs" id="myIconTab" role="tablist">
                        @foreach(config("app.languages") as $key => $language)
                            <li class="nav-item">
                                ...
                                aria-selected="true"><i class="nav-icon i-Add-Cart mr-1"></i>
                                {{ trans('dashboard.slider.Slider Details In '. $language)}}
                                </a>
                            </li>
                        @endforeach
                    </ul>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="form-group row">
                                            <label for="header_one-source-{{ $key }}"
                                                   class="col-sm-2 col-form-label">{{trans('dashboard.slider.Header One')}}</label>
                                            <div class="col-sm-9">
                                                <input type="text" name="{{ $key }}[header_one]"
                                                       value="{{ old("$key.header_one") }}"
                                                       class="form-control @error("$key.header_one") is-invalid @enderror"
                                                id="header_one-source-{{ $key }}"  placeholder="{{trans('dashboard.slider.Enter slider header one')}}">
                                        ...
                                        <div class="form-group row">
                                            <label for="header_two-source-{{ $key }}"
                                                   class="col-sm-2 col-form-label">{{trans('dashboard.slider.Header Two')}}</label>
                                            <div class="col-sm-9">
                                                <input type="text" name="{{ $key }}[header_two]"
                                                       value="{{ old("$key.header_two") }}"
                                                       class="form-control @error("$key.header_two") is-invalid @enderror"
                                                id="header_two-source-{{ $key }}"  placeholder="{{trans('dashboard.slider.Enter slider header two')}}">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-9 text-center ">
                                                <input type="file" name="{{ $key }}[image]" class="dropify" value="{{old('image')}}" data-max-file-size="1M" @if($key == 'en') required @endif />
                                                @if($errors->has('image'))
...
@push('bottom-js')
    ...
    {{--dropzone--}}
    {{--<script src="{{asset('assets/dashboard/js/vendor/dropzone.min.js')}}"></script>--}}
    {{--<script src="{{asset('assets/dashboard/js/dropzone.script.js')}}"></script>--}}
    {{--dropzone--}}
