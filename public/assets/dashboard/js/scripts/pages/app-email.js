  ...
  const showEmail = (id) => {
    if (emailUserList.find('li').length) {
      emailUserList.find('li').on('click', function (e) {
        $(`.email-app-details${id}`).toggleClass('show');
      });
    }
  }
...
// Email Right sidebar toggle
var emailUserList = $('.email-user-list')

const showEmail = (id) => {
  if (emailUserList.find('li').length) {
    console.log('in')
    var emailAppList = $(`.email-app-details${id}`)
    console.log(emailAppList)
    emailAppList.toggleClass('show');
  }
  console.log('out')
}
